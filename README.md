...
ページ番号、注釈種別、注釈の色、コメント本文、作成者を一覧にし、総数と色ごとの件数を表示します。添削のコメントは注釈レイヤーにあるため、ページを画像として見ても`pdftotext`で抽出しても本文には現れません。反映漏れを防ぐために使います。

添削者がハイライトの色で指摘の種類を分けている場合、色が分からないとコメント本文のない注釈の意図を判別できません。色が複数使われているときは、凡例を示すノート注釈を先に探してください。
